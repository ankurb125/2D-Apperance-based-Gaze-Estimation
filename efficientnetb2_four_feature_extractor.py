# -*- coding: utf-8 -*-
"""EfficientNetB2_four_feature_extractor.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/13d7LKEIQg89vxOAMJ77Nek336aOAD6Co
"""
...
def process_images(df,inputPath):
    ...
    for i in range(len(df)):
        ...
        for i in range(0,len(imgPaths),4):
            ...
            fimage = cv2.resize(fimage, (64,64))
            limage = cv2.imread(imgPaths[i+1])
            limage = cv2.resize(limage, (64, 64))
            oimage = cv2.imread(imgPaths[i+2])
            oimage = cv2.resize(oimage, (64, 64))
            rimage = cv2.imread(imgPaths[i+3])
            rimage = cv2.resize(rimage, (64, 64))
...
b1 = tf.keras.applications.EfficientNetB2(weights='imagenet', include_top=False, input_shape=(64, 64, 3))
b2 = tf.keras.applications.EfficientNetB2(weights='imagenet', include_top=False, input_shape=(64, 64, 3))
b3 = tf.keras.applications.EfficientNetB2(weights='imagenet', include_top=False, input_shape=(64, 64, 3))
b4 = tf.keras.applications.EfficientNetB2(weights='imagenet', include_top=False, input_shape=(64, 64, 3))

for layer in b1.layers:
    layer.trainable = True
    layer._name = layer._name + '_eff_1'
for layer in b2.layers:
    layer.trainable = True
    layer._name = layer._name + '_eff_2'
for layer in b3.layers:
    layer.trainable = True
    layer._name = layer._name + '_eff_3'
for layer in b4.layers:
    layer.trainable = True
    layer._name = layer._name + '_eff_4'
...
checkpoint = ModelCheckpoint('EfficientNetB2_four_feature_extractor_finetune.h15', monitor= 'val_root_mean_squared_error', mode= 'min', save_best_only = True, verbose= 1)
